...
// Initialize stat breakdown system
    ...
    setupStatClickListeners() {
        // The main.js wireStatNameClicks function handles this now
        console.log("[DEBUG] StatBreakdown setupStatClickListeners - delegating to main.js wireStatNameClicks");
    }
    ...
    updateData() {
        if (window.characterStats) {
            ...
            // Get selected race from window.gearData
            if (window.gearData && window.gearData.race && window.gearData.race.name) {
                this.selectedRace = window.gearData.race;
            }
    ...
    calculateStatBreakdown(statName) {
        console.log('[DEBUG StatBreakdown] Calculating breakdown for:', statName);
        switch (statName) {
            case 'health':
            ...
            case 'meleehit':
                return this.calculateMeleeHitBreakdown();
            case 'meleecrit':
    ...
    calculateAttackPowerBreakdown() {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant.attackpower && Number(enchant.attackpower) > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: Number(enchant.attackpower),
                            type: 'enchant'
                        });
                        breakdown.total += Number(enchant.attackpower);
                    }
                }
            });
        }
    ...
    calculateBlockValueBreakdown() {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant.blockvalue && Number(enchant.blockvalue) > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: Number(enchant.blockvalue),
                            type: 'enchant'
                        });
                        breakdown.total += Number(enchant.blockvalue);
                    }
                }
            });
        }
    ...
    calculateMP5Breakdown() {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant.mp5 && Number(enchant.mp5) > 0) {
                        const convertedMP5 = Math.round(Number(enchant.mp5) * 2 / 5);
                        breakdown.sources.push({
                            source: `${enchant.name} (${enchant.mp5} → ${convertedMP5})`,
                            value: convertedMP5,
                            type: 'enchant'
                        });
                        breakdown.total += convertedMP5;
    ...
    calculateMeleeHitBreakdown() {
        const breakdown = { total: 0, sources: [], derivedSources: [] };
        
        // Melee hit comes primarily from gear
        Object.entries(this.equippedItems).forEach(([slot, item]) => {
            if (item.meleehit) {
                breakdown.sources.push({
                    source: item.name,
                ...
                breakdown.total += Number(item.meleehit);
            }
        });
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant.meleehit && Number(enchant.meleehit) > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: Number(enchant.meleehit),
                            type: 'enchant'
                        });
                        breakdown.total += Number(enchant.meleehit);
                    }
                }
            });
        }
        ...
        if (this.talentBonuses.meleehit > 0) {
            breakdown.sources.push({
                source: 'Talent Bonuses',
                value: this.talentBonuses.meleehit,
                type: 'talent'
            });
            breakdown.total += this.talentBonuses.meleehit;
        }
    ...
    calculateMeleeCritBreakdown() {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant.meleecrit && Number(enchant.meleecrit) > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: Number(enchant.meleecrit),
                            type: 'enchant'
                        });
                        breakdown.total += Number(enchant.meleecrit);
                    }
                }
            });
        }
    ...
    calculateSpellCritBreakdown() {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant.spellcrit && Number(enchant.spellcrit) > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: Number(enchant.spellcrit),
                            type: 'enchant'
                        });
                        breakdown.total += Number(enchant.spellcrit);
                    }
                }
            });
        }
    ...
    calculateDodgeBreakdown() {
        ...
        const currentAgility = this.characterStats?.currentStats?.agility || 0;
        const currentDefense = this.characterStats?.currentStats?.defense || 0;
        ...
        if (currentDefense > 300) {
            const defenseAbove300 = currentDefense - 300;
            const dodgeFromDefense = defenseAbove300 * 0.04;
            breakdown.derivedSources.push({
                source: `Defense Conversion (${defenseAbove300} defense above 300)`,
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant.dodge && Number(enchant.dodge) > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: Number(enchant.dodge),
                            type: 'enchant'
                        });
                        breakdown.total += Number(enchant.dodge);
                    }
                }
            });
        }
    ...
    calculateThornsBreakdown() {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    let enchantTotalThorns = 0;
                    const enchantThornsVariants = [];
                    
                    if (enchant.thorns && Number(enchant.thorns) > 0) {
                        enchantThornsVariants.push(`${enchant.thorns} Thorns`);
                        enchantTotalThorns += Number(enchant.thorns);
                    }
                    if (enchant.arcanethorns && Number(enchant.arcanethorns) > 0) {
                        enchantThornsVariants.push(`${enchant.arcanethorns} Arcane Thorns`);
                        enchantTotalThorns += Number(enchant.arcanethorns);
                    }
                    if (enchant.firethorns && Number(enchant.firethorns) > 0) {
                        enchantThornsVariants.push(`${enchant.firethorns} Fire Thorns`);
                        enchantTotalThorns += Number(enchant.firethorns);
                    }
                    if (enchant.naturethorns && Number(enchant.naturethorns) > 0) {
                        enchantThornsVariants.push(`${enchant.naturethorns} Nature Thorns`);
                        enchantTotalThorns += Number(enchant.naturethorns);
                    }
                    if (enchant.shadowthorns && Number(enchant.shadowthorns) > 0) {
                        enchantThornsVariants.push(`${enchant.shadowthorns} Shadow Thorns`);
                        enchantTotalThorns += Number(enchant.shadowthorns);
                    }
                    if (enchant.frostthorns && Number(enchant.frostthorns) > 0) {
                        enchantThornsVariants.push(`${enchant.frostthorns} Frost Thorns`);
                        enchantTotalThorns += Number(enchant.frostthorns);
                    }
                    
                    if (enchantTotalThorns > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: enchantTotalThorns,
                            type: 'enchant',
                            breakdown: enchantThornsVariants.join(', ')
                        });
                        breakdown.total += enchantTotalThorns;
                    }
                }
            });
        }
    ...
    calculateSpellstrikeBreakdown() {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    let enchantTotalSpellstrike = 0;
                    const enchantSpellstrikeVariants = [];
                    
                    if (enchant.spellstrike && Number(enchant.spellstrike) > 0) { enchantTotalSpellstrike += Number(enchant.spellstrike); enchantSpellstrikeVariants.push('Spellstrike'); }
                    if (enchant.arcanespellstrike && Number(enchant.arcanespellstrike) > 0) { enchantTotalSpellstrike += Number(enchant.arcanespellstrike); enchantSpellstrikeVariants.push('Arcane'); }
                    if (enchant.firespellstrike && Number(enchant.firespellstrike) > 0) { enchantTotalSpellstrike += Number(enchant.firespellstrike); enchantSpellstrikeVariants.push('Fire'); }
                    if (enchant.naturespellstrike && Number(enchant.naturespellstrike) > 0) { enchantTotalSpellstrike += Number(enchant.naturespellstrike); enchantSpellstrikeVariants.push('Nature'); }
                    if (enchant.shadowspellstrike && Number(enchant.shadowspellstrike) > 0) { enchantTotalSpellstrike += Number(enchant.shadowspellstrike); enchantSpellstrikeVariants.push('Shadow'); }
                    if (enchant.frostspellstrike && Number(enchant.frostspellstrike) > 0) { enchantTotalSpellstrike += Number(enchant.frostspellstrike); enchantSpellstrikeVariants.push('Frost'); }
                    
                    if (enchantTotalSpellstrike > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: enchantTotalSpellstrike,
                            type: 'enchant',
                            breakdown: `(${enchantSpellstrikeVariants.join(' + ')} Spellstrike)`
                        });
                        breakdown.total += enchantTotalSpellstrike;
                    }
                }
            });
        }
    ...
    calculateResistanceBreakdown(statName) {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    let enchantResistanceValue = 0;
                    
                    possibleProperties.forEach(prop => {
                        if (enchant[prop] && Number(enchant[prop]) > 0) {
                            enchantResistanceValue += Number(enchant[prop]);
                        }
                    });
                    
                    if (enchantResistanceValue > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: enchantResistanceValue,
                            type: 'enchant'
                        });
                        breakdown.total += enchantResistanceValue;
                    }
                }
            });
        }
    ...
    calculateWeaponSkillBreakdown(weaponSkillProperty) {
        const breakdown = { total: 0, sources: [], derivedSources: [] }; // No base skill
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchantId]) => {
                if (enchantId && window.enchantDatabase && window.enchantDatabase[enchantId]) {
                    const enchant = window.enchantDatabase[enchantId];
                    if (enchant[weaponSkillProperty] && Number(enchant[weaponSkillProperty]) > 0) {
                        breakdown.sources.push({
                            source: enchant.name,
                            value: Number(enchant[weaponSkillProperty]),
                            type: 'enchant'
                        });
                        breakdown.total += Number(enchant[weaponSkillProperty]);
                    }
                }
            });
        }
    ...
    calculateGenericStatBreakdown(statName) {
        ...
        if (window.gearData && window.gearData.enchants) {
            Object.entries(window.gearData.enchants).forEach(([slot, enchant]) => {
                if (enchant && enchant.stats && enchant.stats[statName]) {
                    breakdown.sources.push({
                        source: `${enchant.name} (Enchant)`,
                        value: enchant.stats[statName],
                        type: 'enchant'
                    });
                    breakdown.total += Number(enchant.stats[statName]);
                }
            });
        }
    ...
    generateBreakdownHTML(statName, breakdown) {
        ...
        let html = `
            <div class='total-stat'>
                Actual ${displayName}: ${breakdown.total.toFixed(2)}${unit}
            </div>
        `;
...
// --- StatBreakdown initialization ---
// Make sure to expose the StatBreakdown class globally
window.StatBreakdown = StatBreakdown;

// Initialize StatBreakdown when the page loads
document.addEventListener('DOMContentLoaded', function() {
    // Major debugging only
    if (!window.statBreakdown && window.StatBreakdown) {
        window.statBreakdown = new StatBreakdown();
        window.statBreakdown.initialize();
    }
});

// Backwards compatibility for older code
window.triggerStatBreakdownInit = function() {
    if (!window.statBreakdown && window.StatBreakdown) {
        window.statBreakdown = new StatBreakdown();
        window.statBreakdown.initialize();
    }
